...
class PBEEDecoder(nn.TransformerDecoder):
    ...
    def __init__(self, decoder_layer, num_layers, 
                 norm=None, patient=1, inner_classifiers_config=None):
...
class DecoderLayer(nn.TransformerDecoderLayer):
    ...
    def forward(self, tgt: torch.Tensor, memory: torch.Tensor, tgt_mask = None,
                memory_mask = None, tgt_key_padding_mask = None,
                memory_key_padding_mask = None, tgt_is_causal = False,
                memory_is_causal = False) -> torch.Tensor:
        global isChecked
        if not isChecked:
            isChecked = True
